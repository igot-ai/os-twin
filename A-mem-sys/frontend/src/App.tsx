...
import { matchesNode, type VaultDataset, type VaultGroup, type VaultNode, vaultData } from "@/lib/mock-vault";
...
type ConnectionMode = "loading" | "mcp" | "demo" | "error";
      ...
      <div className="flex flex-col gap-3 lg:min-w-[420px] lg:max-w-[520px] lg:flex-1">
        ...
        <div className="flex flex-wrap gap-2">
          <Badge variant="secondary">
            {visibleCount} visible notes
          </Badge>
          <Badge variant="secondary">
            {linkCount} connections
          </Badge>
          <Badge variant="secondary">
            {deferredSearchQuery ? `${matchCount} matches` : "Live graph"}
          </Badge>
          <Badge variant="secondary">
            {connectionLabel}
          </Badge>
          ...
          {pinnedNotes.map((note) => (
            <button
              ...
              className={cn(
                ...
                activeNodeId === note.id && "border-primary/50 bg-primary/10",
              )}
...
function GraphMain({
  ...
  connectionMode: ConnectionMode;
  activeNode: VaultNode | null;
  relatedNotes: VaultNode[];
  onSelectNode: (id: string) => void;
...
function ControlsSidebar({
  ...
  settings: GraphSettings;
  activeNode: VaultNode | null;
  visibleNodes: VaultNode[];
  matchingNodes: VaultNode[];
  onToggleGroup: (groupId: string) => void;
  onRestoreGroups: () => void;
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <Button size="sm" onClick={onRestoreGroups}>
              Restore groups
            </Button>
          </div>
        </CardHeader>
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => onUpdateSetting("arrows", checked)}
                    />
                  </div>
                  ...
                  <ControlSlider
                    ...
                    onChange={(value) => onUpdateSetting("textFadeThreshold", value)}
                  />
                  <ControlSlider
                    label="Node size"
                    value={settings.nodeSize}
                    onChange={(value) => onUpdateSetting("nodeSize", value)}
                  />
                  <ControlSlider
                    label="Link thickness"
                    value={settings.linkThickness}
                    onChange={(value) => onUpdateSetting("linkThickness", value)}
                  />

                  <Button
                    className="w-full"
                    variant={settings.animate ? "default" : "secondary"}
                    onClick={() => onUpdateSetting("animate", !settings.animate)}
                  >
              ...
              <AccordionContent>
                <div className="space-y-5">
                  <ControlSlider
                    ...
                    onChange={(value) => onUpdateSetting("centerForce", value)}
                  />
                  <ControlSlider
                    label="Repel force"
                    value={settings.repelForce}
                    onChange={(value) => onUpdateSetting("repelForce", value)}
                  />
                  <ControlSlider
                    label="Link force"
                    value={settings.linkForce}
                    onChange={(value) => onUpdateSetting("linkForce", value)}
                  />
                  <ControlSlider
                    label="Link distance"
                    value={settings.linkDistance}
                    onChange={(value) => onUpdateSetting("linkDistance", value)}
                  />
                </div>
              </AccordionContent>
            </AccordionItem>
        ...
        <CardContent className="space-y-4">
          <div className="rounded-2xl border border-border/70 bg-white/3 p-4">
            <p className="text-xs uppercase tracking-[0.22em] text-muted-foreground">
              {activeNode?.groupId.replaceAll("-", " ") ?? "vault"}
            </p>
...
type ControlSliderProps = Readonly<{
  label: string;
  value: number;
