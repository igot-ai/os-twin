...
class MacOSKeychainVault(VaultStore):
    ...
    def set(self, server: str, key: str, value: str):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to set keychain password: {e.stderr.decode().strip()}"
            )
...
class EncryptedFileVault(VaultStore):
    def __init__(self, path: Path):
        if not CRYPTOGRAPHY_AVAILABLE:
            raise RuntimeError(
                "The 'cryptography' package is required for the encrypted file vault. "
    ...
    def _get_encryption_key(self) -> bytes:
        ...
        if not env_key:
            raise RuntimeError(
                "OSTWIN_VAULT_KEY environment variable is required for the encrypted file vault. "
                'Generate one with: python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())" '
                "and set it in ~/.ostwin/.env"
            )
        try:
            # Fernet key must be 32 bytes and base64 encoded
            return base64.urlsafe_b64encode(env_key.encode().ljust(32)[:32])
        except Exception as e:
            raise RuntimeError(f"Invalid OSTWIN_VAULT_KEY: {e}")
