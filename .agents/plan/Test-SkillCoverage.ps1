        ...
        if ($installDir) {
            ...
            if (Test-Path $dynamicRunner) {
                ...
                $roleDesc = if ($entry.Objective) { $entry.Objective }
                            ...
                            else { "$primaryRole specialist agent" }
...
You are a **$primaryRole** specialist agent.
...
"@ | Out-File -FilePath (Join-Path $rolePath "ROLE.md") -Encoding utf8
                ...
                if (Test-Path $registryPath) {
                    ...
                    $alreadyRegistered = $registry.roles | Where-Object { $_.name -eq $primaryRole }
                    if (-not $alreadyRegistered) {
                        $newEntry = [PSCustomObject]@{
                            name               = $primaryRole
                            description         = $roleDesc
                            runner             = "roles/_base/Start-DynamicRole.ps1"
                            definition         = "roles/$primaryRole/role.json"
                            prompt             = "roles/$primaryRole/ROLE.md"
                ...
                $scaffoldedRoles.Add($primaryRole)
                $scaffolded = $true
                Write-Host "[PRE-FLIGHT] Auto-scaffolded dynamic role '$primaryRole' for $($entry.TaskRef)" -ForegroundColor Yellow
            }
        }
