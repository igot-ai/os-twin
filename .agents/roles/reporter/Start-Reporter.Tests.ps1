...
BeforeAll {
    $script:StartReporter = Join-Path $PSScriptRoot "Start-Reporter.ps1"
...
"@ | Out-File (Join-Path $script:roomDir "brief.md") -Encoding utf8
    ...
    Context "Reporter prompt assembly" {
        ...
        It "role.json has correct capabilities" {
            $roleJson = Get-Content (Join-Path $PSScriptRoot "role.json") -Raw | ConvertFrom-Json
    ...
    Context "Python module availability" {
        It "reporter Python package has __main__.py" {
            Test-Path (Join-Path $PSScriptRoot "__main__.py") | Should -BeTrue
        }

        It "reporter Python package has cli.py" {
            Test-Path (Join-Path $PSScriptRoot "cli.py") | Should -BeTrue
        }

        It "reporter Python package has engine.py" {
            Test-Path (Join-Path $PSScriptRoot "engine.py") | Should -BeTrue
        }

        It "reporter Python package has components.py" {
            Test-Path (Join-Path $PSScriptRoot "components.py") | Should -BeTrue
        }

        It "reporter Python package has brand.json" {
            Test-Path (Join-Path $PSScriptRoot "brand.json") | Should -BeTrue
        }
    }
