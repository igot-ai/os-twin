Describe "Resolve-RoleSkills" {
    BeforeAll {
        $testSkillsDir = Join-Path (Resolve-Path "$PSScriptRoot/../../../roles/_base").Path "test_skills"
        $testRolePath = Join-Path (Resolve-Path "$PSScriptRoot/../../../roles/_base").Path "test_role"
    ...
    It "resolves all tiers for an engineer role" {
        $skills = & (Join-Path (Resolve-Path "$PSScriptRoot/../../../roles/_base").Path "Resolve-RoleSkills.ps1") -RoleName "engineer" -RolePath $testRolePath -SkillsBaseDir $testSkillsDir
    ...
    It "throws error for missing explicit skill" {
        ...
        { & (Join-Path (Resolve-Path "$PSScriptRoot/../../../roles/_base").Path "Resolve-RoleSkills.ps1") -RoleName "engineer" -RolePath $testRolePath -SkillsBaseDir $testSkillsDir } | Should -Throw "Skill Not Found*"
    }
}
