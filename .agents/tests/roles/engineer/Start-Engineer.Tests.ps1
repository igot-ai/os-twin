...
BeforeAll {
    ...
    $script:PostMessage = Join-Path (Split-Path (Resolve-Path "$PSScriptRoot/../../../roles/engineer").Path) ".." "channel" "Post-Message.ps1"
    $script:ReadMessages = Join-Path (Split-Path (Resolve-Path "$PSScriptRoot/../../../roles/engineer").Path) ".." "channel" "Read-Messages.ps1"
