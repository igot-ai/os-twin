$content = Get-Content /Users/paulaan/PycharmProjects/agent-os/.agents/plan/Start-Plan.ps1 -Raw

$newContent = $content -replace '(?s)# --- Plan Expansion \(Manager Loop Startup Integration\).*?(?=# --- Extract plan_id from embedded config ---)', '
...
if ($shouldExpand -and ($PlanFile -notmatch ''\.refined\.md$'')) {
    ...
    if (Test-Path $expandScript) {
        ...
        if (-not $DryRun -and (Test-Path $refinedFile)) {
            ...
            Write-Log -Message "Plan expansion diff:`n$diffSummary" -Level "INFO" -Source "manager"
...
'
$newContent | Out-File -FilePath /Users/paulaan/PycharmProjects/agent-os/.agents/plan/Start-Plan.ps1 -Encoding utf8 -NoNewline
