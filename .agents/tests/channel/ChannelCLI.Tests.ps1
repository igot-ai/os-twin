...
# Updated for argparse-based channel CLI (list, connect, disconnect, test, pair).
...
Describe "ostwin channel help and usage" {
    ...
    It "shows help with --help flag" {
        $result = bash $script:ostwin channel --help 2>&1
...
Describe "ostwin channel subcommands" {
    It "list subcommand exists" {
        $result = bash $script:ostwin channel list --help 2>&1
        $joined = ($result | Out-String)
        $joined | Should -Match "list"
    }
}
