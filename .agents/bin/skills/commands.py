...
def _list(
    ...
    # Display as table (Epic 4 requirement)
    table = Table(title="Agent OS Skills", show_header=True, header_style="bold magenta")
    ...
    for skill in skills:
        ...
        trust = skill.get("trust_level", "experimental")
        source = skill.get("source", "unknown")
        
        # Color the source
        source_styled = source
        if source == "built-in":
            source_styled = f"[magenta]{source}[/magenta]"
        elif source == "project":
            source_styled = f"[green]{source}[/green]"
        elif source == "user":
            source_styled = f"[cyan]{source}[/cyan]"
