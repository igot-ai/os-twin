...
switch ($Command) {
    ...
    "dashboard" {
        ...
        if (Test-Path $dashPs1) {
            ...
            Write-Error "[ERROR] dashboard.ps1 not found."
            exit 1
        }
    }
    ...
    "clone-role" {
        ...
        & $ostwinExec role manager clone -RoleName $roleName -ProjectDir $cloneProjectDir
    }
    ...
    "mcp" {
        ...
        $mcpPs1 = Join-Path $AgentsDir "mcp\MCP-Extension.ps1"
        $mcpSh = Join-Path $AgentsDir "mcp\mcp-extension.sh"
        if (Test-Path $mcpPs1) {
            $mcpArgs = [System.Collections.ArrayList]::new()
            if ($mcpProjectRoot) {
                $null = $mcpArgs.Add('-ProjectDir')
                $null = $mcpArgs.Add($mcpProjectRoot)
            }
            foreach ($a in $Arguments) { $null = $mcpArgs.Add($a) }
            & pwsh -NoProfile -File $mcpPs1 @mcpArgs
        }
        elseif (Test-Path $mcpSh) {
            Write-Error "[ERROR] MCP-Extension.ps1 not found. Only mcp-extension.sh is available."
            Write-Error "  Port mcp-extension.sh to PowerShell or run: bash $mcpSh $($Arguments -join ' ')"
            exit 1
        }
        else {
            Write-Error "[ERROR] MCP extension script not found."
            exit 1
        }
    }
    ...
    # ── test ─────────────────────────────────────────────────────────────────
    "test" {
        # Prefer PowerShell test runner, fall back to Pester directly
        $testPs1 = Join-Path $AgentsDir "tests\Run-All.ps1"
        if (Test-Path $testPs1) {
            & pwsh -NoProfile -File $testPs1 @Arguments
        }
        else {
            # Inline Pester invocation as fallback
