"""Agent OS CLI — role-aware wrapper around deepagents_cli.

This module overrides deepagents_cli's skill loading to inject role-specific
skills from AGENT_OS_SKILLS_DIR (set by Invoke-Agent.ps1). Each war-room
agent gets only the skills relevant to its assigned role.

Environment variables consumed (set by Invoke-Agent.ps1):
    AGENT_OS_SKILLS_DIR: Absolute path to role-isolated skills directory
    AGENT_OS_ROLE:       Role name (engineer, qa, architect, etc.)
    AGENT_OS_ROOM_DIR:   Path to the war-room directory

Skill precedence (lowest → highest):
    built-in → user ~/.deepagents → user ~/.agents
    → project .deepagents → project .agents/skills
    → AGENT_OS_SKILLS_DIR (role-isolated, highest)
"""
...
# ---------------------------------------------------------------------------
# Monkey-patch Settings BEFORE any deepagents_cli code runs, so that when
# create_agent() calls settings.get_project_agent_skills_dir(), it returns
# the role-isolated skills directory instead of the generic project skills.
# ---------------------------------------------------------------------------
from deepagents_cli.config import Settings  # noqa: E402

_original_get_project_agent_skills = Settings.get_project_agent_skills_dir


def _get_role_aware_skills_dir(self) -> "Path | None":
    """Return AGENT_OS_SKILLS_DIR if set (role-isolated), else fallback."""
    skills_dir = os.environ.get("AGENT_OS_SKILLS_DIR")
    if skills_dir:
        p = Path(skills_dir)
        if p.is_dir():
            return p
    return _original_get_project_agent_skills(self)
...
def parse_args() -> argparse.Namespace:
    ...
    # --- Extend skills parser with ostwin-local API subcommands ---
    skills_parser = subparsers.choices.get("skills")
    if skills_parser:
        skills_sub = None
        for action in skills_parser._subparsers._actions:
            if isinstance(action, argparse._SubParsersAction):
                skills_sub = action
                break
        if skills_sub:
            api_search = skills_sub.add_parser(
                "api-search",
                help="Search skills via dashboard API",
            )
            api_search.add_argument("query", help="Search query")
            api_search.add_argument("--role", default=None, help="Filter by role")
            api_search.add_argument("--tags", default="", help="Comma-separated tags")

            api_list = skills_sub.add_parser(
                "api-list",
                help="List all skills via dashboard API",
            )
            api_list.add_argument("--role", default=None, help="Filter by role")

            api_install = skills_sub.add_parser(
                "api-install",
                help="Install a skill from disk via dashboard API",
            )
            api_install.add_argument("path", help="Absolute path to skill directory")

            api_sync = skills_sub.add_parser(
                "api-sync",
                help="Sync skills disk → vector store via dashboard API",
            )
...
def _execute_ostwin_skills_api(args: argparse.Namespace, sub: str) -> None:
    """Execute ostwin-local skills API subcommands against the dashboard REST API."""
    import requests

    base = os.environ.get("OSTWIN_DASHBOARD_URL", "http://localhost:9000")
        ...
        if sub == "api-search":
            ...
            if getattr(args, "tags", ""):
                for t in args.tags.split(","):
                    t = t.strip()
                    if t:
                        params.setdefault("tags", [])  # type: ignore[arg-type]
                        # requests handles list params
            resp = requests.get(f"{base}/api/skills/search", params=params, timeout=10)
            resp.raise_for_status()
            skills = resp.json()
            if not skills:
                print("No skills found.")  # noqa: T201
            else:
                for s in skills:
                    tags = ", ".join(s.get("tags", []))
                    print(f"  {s['name']:20s}  {s.get('description', '')[:60]}  [{tags}]")  # noqa: T201

        elif sub == "api-list":
            params = {}
            if getattr(args, "role", None):
                params["role"] = args.role
            resp = requests.get(f"{base}/api/skills", params=params, timeout=10)
            resp.raise_for_status()
            skills = resp.json()
            if not skills:
                print("No skills found.")  # noqa: T201
                ...
                for s in skills:
                    ...
                    print(f"  {s['name']:20s}  [{trust:12s}]  {s.get('description', '')[:50]}  [{tags}]")  # noqa: T201

        elif sub == "api-install":
            resp = requests.post(
            ...
            resp.raise_for_status()
            result = resp.json()
            print(f"✓ Installed skill: {result.get('skill', 'unknown')}")  # noqa: T201

        elif sub == "api-sync":
            resp = requests.post(f"{base}/api/skills/sync", timeout=30)
            resp.raise_for_status()
            result = resp.json()
            print(f"Synced {result.get('synced_count', 0)} skills")  # noqa: T201
            if result.get("added"):
                print(f"  Added:   {', '.join(result['added'])}")  # noqa: T201
            if result.get("updated"):
                print(f"  Updated: {', '.join(result['updated'])}")  # noqa: T201
            if result.get("removed"):
                print(f"  Removed: {', '.join(result['removed'])}")  # noqa: T201

    except requests.ConnectionError:
        print(f"✗ Cannot connect to dashboard at {base}. Is the server running?")  # noqa: T201
        sys.exit(1)
    except requests.HTTPError as e:
        print(f"✗ API error: {e.response.status_code} — {e.response.text[:200]}")  # noqa: T201
        sys.exit(1)
...
def cli_main() -> None:
    ...
    try:
        ...
        if args.command == "help":
            ...
            skills_sub = getattr(args, "skills_command", None)
            if skills_sub and skills_sub.startswith("api-"):
                _execute_ostwin_skills_api(args, skills_sub)
            else:
...
# ---------------------------------------------------------------------------
# Also inject the role name as agent_id so skills, memory, and subagents
# are scoped per-role rather than all using the default "agent" identity.
# ---------------------------------------------------------------------------
...
def cli_main() -> None:
    ...
    When AGENT_OS_ROLE is set:
    - Skills are loaded from the role-isolated directory (AGENT_OS_SKILLS_DIR)
    - The agent identity is set to the role name (e.g. "engineer", "qa")
