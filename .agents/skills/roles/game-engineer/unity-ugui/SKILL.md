...
name: unity-ugui
description: >-
  Professional Unity UI (uGUI) assistant. Masters responsive layouts, TextMeshPro,
  reactive UI patterns (UniRx/UniTask), UI animation via Animator + PrimeTween,
  TextMeshPro (merged), and the Event System. Use for building screens, HUDs, popups,
  or optimizing UI performance. Always uses `unity-editor` sub-skills for all Editor
  operations and `unity-code-review` for script changes.
---
...
4. animation-modify    → Set keyframes (e.g., CanvasGroup.alpha: 0→1 for Open, 1→0 for Closed)
...
Step 4: gameobject-component-modify → Configure CanvasScaler (Scale With Screen Size, 1080×1920, Match 0.5)
