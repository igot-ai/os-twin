...
def _normalize_plan_header(content: str, title: str) -> str:
    ...
    if header_match:
        ...
        if header_match.group(1) == "PLAN":
            content = _re_mod.sub(
                r"^# PLAN:", "# Plan:", content, count=1, flags=_re_mod.MULTILINE
            )
        return content
...
def _read_plan_meta(plan_id: str) -> Dict[str, Any]:
    ...
    if not meta_file.exists():
        return {}
    try:
        return read_json_utf8(meta_file)
    except (json.JSONDecodeError, OSError):
        return {}


def _write_plan_meta(plan_id: str, meta: Dict[str, Any]) -> None:
    _plan_meta_path(plan_id).write_text(
...
def _get_valid_epic_refs(plan_id: str) -> set:
    ...
    return set(
        _re_mod.findall(
            r"^#{2,3}\s+(?:(?:Epic|Task):\s*)?(EPIC-\d+)", content, _re_mod.MULTILINE
        )
    )
...
def _inject_assets_to_working_directory(plan_id: str, working_dir: Path) -> None:
    ...
    for asset in assets:
        ...
        try:
            if file_size > LARGE_FILE_THRESHOLD:
                ...
                logger.info(
                    f"Symlinked large asset ({file_size} bytes): {original_name}"
                )
            else:
                # Copy small files
    ...
    logger.info(
        f"Injected {injected_count} assets into {assets_target_dir} for plan {plan_id}"
    )
        ...
        with zipfile.ZipFile(io.BytesIO(zip_data), "r") as zf:
            for zip_info in zf.infolist():
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to extract {filename} from {original_zip_name}: {e}"
                    )
                    continue
...
def _merge_markdown_asset_edits_into_meta(plan_id: str, markdown_content: str) -> None:
    ...
    for epic_ref, entries in parsed.items():
        for entry in entries:
            ...
            original_name = (
                raw_name[len(".assets/") :]
                if raw_name.startswith(".assets/")
                else raw_name
            )

            asset = assets_by_original.get(original_name) or assets_by_filename.get(
                original_name
            )
            if asset:
                fn = asset["filename"]
...
def bind_asset_to_epic(plan_id: str, filename: str, epic_ref: str) -> Dict[str, Any]:
...
def list_epic_assets(
    plan_id: str, epic_ref: str, *, validate: bool = True
) -> List[Dict[str, Any]]:
    if validate:
        _validate_epic_ref(plan_id, epic_ref)
    meta = _ensure_plan_meta(plan_id)
...
def get_asset_or_404(plan_id: str, filename: str) -> Dict[str, Any]:
    ...
    for asset in meta.get("assets", []):
        if asset.get("filename") == filename:
            return asset
    raise HTTPException(
        status_code=404, detail=f"Asset {filename} not found in plan {plan_id}"
    )
...
def _update_plan_assets_section(
    plan_id: str, all_assets: list, assets_dir: Path
) -> None:
    ...
    if _re_mod.search(assets_pattern, content, _re_mod.MULTILINE):
        content = _re_mod.sub(
            assets_pattern, assets_block, content, count=1, flags=_re_mod.MULTILINE
        )
    else:
...
def _update_epic_asset_sections(
    plan_id: str, all_assets: list, assets_dir: Path
) -> None:
    ...
    for i in range(len(matches) - 1, -1, -1):
        ...
        if epic_assets:
            ...
            if skills_match:
                ...
                epic_block = (
                    epic_block[:insert_pos] + asset_line + epic_block[insert_pos:]
                )
            else:
...
async def list_plan_assets(plan_id: str, user: dict = Depends(get_current_user)):
    ...
    meta = _ensure_plan_meta(plan_id)
    assets = [
        _serialize_plan_asset(plan_id, asset)
        for asset in _normalize_plan_assets(plan_id, meta)
    ]
...
async def upload_plan_assets(
    ...
    for upload in files:
        ...
        is_zip = content_type in [
            ...
            "application/x-zip",
        ] or original_name.lower().endswith(".zip")
    ...
    if extracted_count > 0:
        logger.info(
            f"Extracted {extracted_count} files from ZIP archives for plan {plan_id}"
        )
...
async def api_unbind_asset(
    plan_id: str, filename: str, epic_ref: str, user: dict = Depends(get_current_user)
):
...
async def api_list_epic_assets(
    plan_id: str, epic_ref: str, user: dict = Depends(get_current_user)
):
...
async def api_update_asset_metadata(
    plan_id: str,
    filename: str,
    request: Request,
    user: dict = Depends(get_current_user),
...
async def download_asset(
    plan_id: str, filename: str, user: dict = Depends(get_current_user)
):
    ...
    _normalize_plan_assets(plan_id, meta)
    asset = next(
        (a for a in meta.get("assets", []) if a.get("filename") == filename), None
    )
    mime = (
...
async def generate_plan_from_assets(
    ...
    for asset in assets:
        ...
        is_text = any(mime.startswith(prefix) for prefix in TEXT_MIME_PREFIXES) or any(
...
Create a detailed, concrete plan that directly uses the information in these files."""
    ...
    try:
        ...
        if not parsed.get("plan"):
            raise HTTPException(
                status_code=500, detail="AI failed to generate a valid plan"
            )
        ...
        title_match = re.search(
            r"^# (?:Plan|PLAN):\s*(.+)", parsed["plan"], re.MULTILINE
        )
...
async def list_plans(
    ...
    order_by: str = Query(
        "created_desc",
        description="Sort order: created_desc, created_asc, title, time_id",
    ),
    user: dict = Depends(get_current_user),
):
    ...
    for f in sorted(
        ...
        if plan_id in zvec_plans:
            ...
            epics_found = re.findall(
                r"^#{2,3} (?:(?:Epic|Task):\s*\S+|EPIC-\d+|TASK-\d+)",
                content,
                re.MULTILINE,
            )
            ...
            if store:
                try:
                    ...
                    logger.warning(
                        "Failed to backfill plan %s into zvec: %s", plan_id, e
                    )
        ...
        if os.environ.get("NEXT_PUBLIC_ENABLE_MOCK_REALTIME") == "true":
            ...
            p["completed_epics"] = random.randint(
                0, max(0, p.get("epic_count", 5) - p["active_epics"])
            )
...
async def get_stats(user: dict = Depends(get_current_user)):
    ...
    full_history = history + [
        {"timestamp": datetime.now(timezone.utc).isoformat(), **current_stats}
    ]
...
async def reload_plan_from_disk(plan_id: str, user: dict = Depends(get_current_user)):
    ...
    title = title_match.group(1).strip() if title_match else plan_id
    epics_found = re.findall(
        r"^#{2,3} (?:(?:Epic|Task):\s*\S+|EPIC-\d+|TASK-\d+)", content, re.MULTILINE
    )
    epic_count = len(epics_found)
...
async def get_plan_sync_status(plan_id: str, user: dict = Depends(get_current_user)):
    ...
    return {"in_sync": in_sync, "disk_mtime": disk_mtime, "zvec_mtime": zvec_mtime}


def create_plan_on_disk(
    title: str,
    ...
    if not plan_roles_file.exists():
        ...
        seed_config = (
            json.loads(global_config_file.read_text())
...
async def save_plan(
    plan_id: str, request: SavePlanRequest, user: dict = Depends(get_current_user)
):
    plans_dir = PLANS_DIR
    plan_file = plans_dir / f"{plan_id}.md"
    if not plan_file.exists():
        ...
        if store:
            try:
                old_title_match = re.search(
                    r"^# (?:Plan|PLAN):\s*(.+)", old_content, re.MULTILINE
                )
                old_title = (
                    old_title_match.group(1).strip() if old_title_match else plan_id
                )
                old_epics = len(
                    re.findall(
                        r"^#{2,3} (?:(?:Epic|Task):\s*\S+|EPIC-\d+|TASK-\d+)",
    ...
    title_match = re.search(
        r"^# (?:Plan|PLAN):\s*(.+)", normalized_content, re.MULTILINE
    )
    title = title_match.group(1).strip() if title_match else plan_id
...
async def get_plan_roles(plan_id: str, user: dict = Depends(get_current_user)):
    ...
    def _build_role(name: str) -> dict:
        ...
        return {
            ...
            "timeout_seconds": cfg.get(
                "timeout_seconds", reg.get("timeout_seconds", 600)
            ),
    ...
    if effective_names:
        ...
        all_names = list(
            dict.fromkeys(list(plan_config.keys()) + list(role_registry.keys()))
        )
...
async def update_plan_config(
    plan_id: str, config: Dict[str, Any], user: dict = Depends(get_current_user)
):
...
async def attach_skill(
    plan_id: str, skill: Dict[str, str], user: dict = Depends(get_current_user)
):
...
async def detach_skill(
    plan_id: str, skill_name: str, user: dict = Depends(get_current_user)
):
...
async def run_plan(request: RunRequest, user: dict = Depends(get_current_user)):
    ...
    has_epics = bool(
        _re_mod.search(r"^#{2,3} (?:EPIC-|Task:|Epic:)", plan, _re_mod.MULTILINE)
    )
    has_goal = bool(
        _re_mod.search(r"^#\s+(?:Plan|PLAN):\s*.+", plan, _re_mod.MULTILINE)
    )
    if not has_epics and not has_goal:
        raise HTTPException(
            status_code=400,
            detail="Plan must contain a '# Plan: Title' goal or at least one '## EPIC-XXX - Title' section.",
        )
    ...
    if existing_content != plan:
        ...
        if existing_content and existing_content.strip() and store:
            try:
                old_title_match = _re_mod.search(
                    r"^# (?:Plan|PLAN):\s*(.+)", existing_content, _re_mod.MULTILINE
                )
                old_title = (
                    old_title_match.group(1).strip() if old_title_match else plan_id
                )
                ...
                logger.warning(
                    "Failed to snapshot plan version before launch %s: %s", plan_id, e
                )
    ...
    meta = {
        **existing_meta,  # keep previous fields
        ...
        else str(PROJECT_ROOT / working_dir / ".war-rooms"),
        "status": "launched",
    }
    ...
    if not role_config_path.exists():
        ...
        logger.info(
            f"run_plan: seeded roles.json for {plan_id} from engine config + dashboard roles"
        )
    else:
        logger.debug(
            f"run_plan: roles.json already exists for {plan_id}, preserving user customisations"
        )
    ...
    wd_path = (
        Path(working_dir)
        if Path(working_dir).is_absolute()
        else PROJECT_ROOT / working_dir
    )
    if not (wd_path / ".agents").exists():
        logger.info(
            f"run_plan: target dir {wd_path} not initialized, running ostwin init..."
        )
        ...
        if ostwin_bin.exists():
            ...
            if init_result.returncode != 0:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"ostwin init failed in {wd_path}: {init_result.stderr[:200]}",
                )
    ...
    if global_assets_dir.exists() and global_assets_dir.is_dir():
        ...
        for asset_file in global_assets_dir.iterdir():
            if asset_file.is_file():
                ...
                if not dst_file.exists() or str(dst_file.resolve()) != str(
                    asset_file.resolve()
                ):
...
async def get_plan_version(
    plan_id: str, version: int, user: dict = Depends(get_current_user)
):
    ...
    if not v:
        raise HTTPException(
            status_code=404, detail=f"Version {version} not found for plan {plan_id}"
        )
...
async def restore_plan_version(
    ...
    if current_content.strip():
        try:
            cur_title_match = re.search(
                r"^# (?:Plan|PLAN):\s*(.+)", current_content, re.MULTILINE
            )
            ...
            store.save_plan_version(
                ...
                title=cur_title,
                epic_count=cur_epics,
                change_source="before_restore",
            )
        except Exception as e:
    ...
    try:
        title_match = re.search(
            r"^# (?:Plan|PLAN):\s*(.+)", restored_content, re.MULTILINE
        )
        title = title_match.group(1).strip() if title_match else plan_id
        epics_found = re.findall(
            r"^#{2,3} (?:(?:Epic|Task):\s*\S+|EPIC-\d+|TASK-\d+)",
...
async def get_all_goals():
    ...
    for f in plans_dir.glob("*.md"):
        ...
        if goal_section:
            all_goals.append({"plan_id": f.stem, "goal": goal_section.group(1).strip()})
    ...
    try:
        ...
        if request.asset_context:
            ...
            for asset in request.asset_context:
                asset_name = (
                    asset.get("original_name") or asset.get("filename") or "attachment"
                )
        ...
        if request.images:
            images = [
                {"url": img.get("url", "")} for img in request.images if img.get("url")
            ]
            logger.info(
                f"[REFINE] Received {len(images)} image(s) for multimodal processing"
            )
            for i, img in enumerate(images[:3]):  # Log first 3
                url_preview = (
                    img["url"][:100] + "..." if len(img["url"]) > 100 else img["url"]
                )
...
async def get_plan_role_assignments(
...
async def update_plan_role_config(
    ...
    if request.temperature is not None:
        config[role_name]["temperature"] = request.temperature
    ...
    if request.skill_refs is not None:
        config[role_name]["skill_refs"] = request.skill_refs
    if request.disabled_skills is not None:
        config[role_name]["disabled_skills"] = request.disabled_skills
    plan_roles_file.write_text(json.dumps(config, indent=2) + "\n")
    return {
        "status": "updated",
        "plan_id": plan_id,
        "role": role_name,
        "config": config[role_name],
    }
...
async def update_epic_state(
    plan_id: str, epic_ref: str, body: dict, user: dict = Depends(get_current_user)
):
    ...
    if not warrooms_dir.exists():
        raise HTTPException(
            status_code=404, detail=f"War-rooms directory not found for plan {plan_id}"
        )
    ...
    if not target_room_dir:
        raise HTTPException(
            status_code=404,
            detail=f"No war-room found for epic_ref '{epic_ref}' in plan {plan_id}",
        )
            ...
            if cp:
                ...
                for cp_ref in cp:
                    ...
                    if cp_room_id:
                        ...
                        if (
                            cp_status_file.exists()
                            and cp_status_file.read_text().strip() == "passed"
                        ):
                            cp_passed += 1
...
async def get_plan_epic(
    ...
    for room_dir in warrooms_dir.glob("room-*"):
        ...
        if current_ref == task_ref:
            ...
            if plan_file.exists():
                content = plan_file.read_text()
                title_match = re.search(
                    r"^# (?:Plan|PLAN):\s*(.+)", content, re.MULTILINE
                )
                if title_match:
    ...
    raise HTTPException(
        status_code=404, detail=f"EPIC {task_ref} not found in plan {plan_id}"
    )


@router.get("/api/plans/{plan_id}/rooms/{room_id}/roles")
async def get_plan_room_roles(
    plan_id: str, room_id: str, user: dict = Depends(get_current_user)
):
    warrooms_dir = resolve_runtime_plan_warrooms_dir(plan_id)
    if not warrooms_dir:
...
async def get_plan_room_channel(
    ...
    if not warrooms_dir:
        raise HTTPException(
            status_code=404, detail=f"Room {room_id} not found in plan {plan_id}"
        )
    room_dir = warrooms_dir / room_id
    if not room_dir.exists():
        raise HTTPException(
...
async def get_epic_tasks(
    plan_id: str, task_ref: str, user: dict = Depends(get_current_user)
):
    ...
    for line in raw.splitlines():
        ...
        if line_stripped.startswith("- ["):
            completed = line_stripped.startswith("- [x]") or line_stripped.startswith(
                "- [X]"
            )
...
async def get_epic_lifecycle(
    plan_id: str, task_ref: str, user: dict = Depends(get_current_user)
):
    room_dir = _resolve_room_dir(plan_id, task_ref)
    if not room_dir:
        return {"states": {}, "transitions": [], "error": "Room not found"}
    lc_file = room_dir / "lifecycle.json"
    if not lc_file.exists():
        return {"states": {}, "transitions": [], "error": "lifecycle.json not found"}
    try:
        return json.loads(lc_file.read_text())
    except (json.JSONDecodeError, OSError):
        return {"states": {}, "transitions": [], "error": "JSON error"}


@router.get("/api/plans/{plan_id}/epics/{task_ref}/audit")
async def get_epic_audit(
    plan_id: str, task_ref: str, user: dict = Depends(get_current_user)
):
    room_dir = _resolve_room_dir(plan_id, task_ref)
    if not room_dir:
...
async def get_epic_brief(
    plan_id: str, task_ref: str, user: dict = Depends(get_current_user)
):
    room_dir = _resolve_room_dir(plan_id, task_ref)
    if not room_dir:
        return {"content": "", "working_dir": "", "created_at": None}
...
async def get_epic_roles(
    ...
    room_dir = _resolve_room_dir(plan_id, task_ref)
    if not room_dir:
        raise HTTPException(status_code=404, detail="Epic room not found")
...
async def update_epic_role_assignment(
    ...
    return {
        "status": "updated",
        "task_ref": task_ref,
        "candidate_roles": rc["assignment"]["candidate_roles"],
    }
...
async def update_epic_role_config(
    ...
    if "roles" not in rc:
        rc["roles"] = {}
    if role_name not in rc["roles"]:
        rc["roles"][role_name] = {}

    if request.default_model is not None:
        rc["roles"][role_name]["default_model"] = request.default_model
    if request.temperature is not None:
        rc["roles"][role_name]["temperature"] = request.temperature
    if request.timeout_seconds is not None:
        rc["roles"][role_name]["timeout_seconds"] = request.timeout_seconds
    if request.cli is not None:
        rc["roles"][role_name]["cli"] = request.cli
    if request.skill_refs is not None:
        rc["roles"][role_name]["skill_refs"] = request.skill_refs
    if request.disabled_skills is not None:
        rc["roles"][role_name]["disabled_skills"] = request.disabled_skills
