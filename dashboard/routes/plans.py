...
async def get_plan_roles(plan_id: str, user: dict = Depends(get_current_user)):
    ...
    for roles_root in [
        ...
        PROJECT_ROOT / "contributes" / "roles",
    ]:
    ...
    def _build_role(name: str) -> dict:
        ...
        return {
            ...
            "default_model": cfg.get(
                "default_model", reg.get("default_model", reg.get("model", ""))
            ),
            "timeout_seconds": cfg.get(
                "timeout_seconds", reg.get("timeout_seconds", reg.get("timeout", 600))
            ),
...
async def run_plan(request: RunRequest, user: dict = Depends(get_current_user)):
    ...
    if not _re_mod.search(r"^#{2,3} (?:EPIC-|Task:|Epic:)", plan, _re_mod.MULTILINE):
        raise HTTPException(
            status_code=400,
            detail="Plan contains no epics or tasks. Add at least one '## EPIC-XXX - Title' section.",
        )
