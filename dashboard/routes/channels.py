...
def save_channels_config(configs: List[ConnectorConfig]):
    ...
    with open(CHANNELS_CONFIG_PATH, "w", encoding="utf-8") as f:
...
async def list_channels(user: dict = Depends(get_current_user)):
    ...
    for p in platforms:
        ...
        result.append(ChannelStatus(platform=p, status=status, config=config))
    return result
...
async def get_channel(platform: str, user: dict = Depends(get_current_user)):
    ...
    return ChannelStatus(platform=platform, status=status, config=config)
...
async def test_channel(platform: str, user: dict = Depends(get_current_user)):
    ...
    return {
        "status": "healthy",
        "message": f"Connection to {platform} is working (mocked)",
    }


@router.put("/{platform}/settings")
async def update_settings(
    platform: str, settings: Dict[str, Any], user: dict = Depends(get_current_user)
):
    configs = read_channels_config()
    config = next((c for c in configs if c.platform == platform), None)
    ...
    if "notification_preferences" in settings:
        ...
        config.notification_preferences.events = prefs.get(
            "events", config.notification_preferences.events
        )
        config.notification_preferences.enabled = prefs.get(
            "enabled", config.notification_preferences.enabled
        )
