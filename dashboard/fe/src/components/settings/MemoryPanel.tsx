...
export function MemoryPanel({ memory, provenance = {}, onUpdate }: MemoryPanelProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Section 1: AI Model (read-only, configured via Providers + AI namespace) ── */}
      <section>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-xs text-slate-700 font-semibold mb-1">
                Memory LLM calls are routed through the unified AI gateway
              </p>
              <p className="text-[11px] text-slate-500 mb-3">
                The model is configured in <strong>Settings &rarr; Provider Config</strong> (Google provider) 
                and <strong>Settings &rarr; AI Monitor</strong> (per-purpose overrides).
                All calls go through <code className="font-mono text-[10px] bg-white px-1 py-0.5 rounded">shared/ai.get_completion(purpose=&quot;memory&quot;)</code>.
              </p>
              <div className="flex flex-wrap gap-2">
                ...
                <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-white border border-slate-200 text-[10px] font-mono text-slate-600">
                  <span className="material-symbols-outlined text-[12px] text-blue-600">monitoring</span>
                  Monitored via AI Monitor
                </span>
              </div>
            </div>
          </div>
