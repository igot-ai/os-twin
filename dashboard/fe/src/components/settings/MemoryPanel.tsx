...
export function MemoryPanel({ memory, provenance = {}, onUpdate, allModels }: MemoryPanelProps) {
  ...
  // Detect if selected model uses cloud auth (no endpoint/key needed)
  const selectedLlmModel = allModels.find((m) => m.id === draft.llm_model);
  const llmUsesCloudAuth = selectedLlmModel?.provider_id === 'google';
  const selectedEmbedModel = allModels.find((m) => m.id === draft.embedding_model);
      ...
      <section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {/* Vertex AI model — no endpoint/key needed */}
            ...
            {/* Other models — show endpoint/key fields */}
            {draft.llm_backend === 'openai-compatible' && !llmUsesCloudAuth && (
