...
def temp_plan_h2(tmp_path, monkeypatch):
    """Plan fixture using ## EPIC-XXX headers (h2 level, as generated by AI agents)."""
    monkeypatch.setattr("dashboard.routes.plans.PLANS_DIR", tmp_path)
    monkeypatch.setattr("dashboard.routes.plans.GLOBAL_PLANS_DIR", tmp_path)
    plan_id = "test-plan-h2"
    plan_file = tmp_path / f"{plan_id}.md"
    plan_file.write_text(
        "# Plan: Test Plan H2\n\n"
        "## EPIC-001 - First\n\n"
...
def test_update_epic_asset_sections_h2_headers(temp_plan_h2):
    """_update_epic_asset_sections should insert > Assets: lines for ## EPIC headers."""
    from dashboard.routes.plans import _sync_asset_sections
    plan_id, tmp_path, assets_dir = temp_plan_h2
    
    (assets_dir / "mockup.png").write_bytes(b"png-data")
    meta = _ensure_plan_meta(plan_id)
    meta["assets"] = [
        {"filename": "mockup.png", "original_name": "mockup.png",
         "mime_type": "image/png", "asset_type": "design-mockup",
         "description": "Design mockup", "bound_epics": ["EPIC-001"]}
    ]
    meta["epic_assets"] = {"EPIC-001": ["mockup.png"]}
    _write_plan_meta(plan_id, meta)
    
    _sync_asset_sections(plan_id)
    
    content = (tmp_path / f"{plan_id}.md").read_text()
    # Should have > Assets: under EPIC-001
    assert "> Assets:" in content
    assert "mockup.png" in content


def test_parse_epic_assets_from_markdown_h2_headers():
    """_parse_epic_assets_from_markdown should find assets from ## EPIC sections."""
    from dashboard.routes.plans import _parse_epic_assets_from_markdown
    
    content = (
        "# Plan: Test\n\n"
        "## EPIC-001 - Build API\n\n"
        "> Assets: .assets/spec.yaml (api-spec, text/yaml) — API spec\n\n"
        "depends_on: []\n\n"
        "## EPIC-002 - Frontend\n\n"
        "depends_on: []\n"
    )
    result = _parse_epic_assets_from_markdown(content)
    assert "EPIC-001" in result
    assert len(result["EPIC-001"]) == 1
    assert "spec.yaml" in result["EPIC-001"][0]


def test_bind_unbind_asset_h2_headers(temp_plan_h2):
    """Asset binding should work the same for ## EPIC headers."""
    plan_id, tmp_path, assets_dir = temp_plan_h2
    (assets_dir / "asset1.txt").write_text("content")
    ...
    # Bind
    bind_asset_to_epic(plan_id, "asset1.txt", "EPIC-001")

    meta = _ensure_plan_meta(plan_id)
    assert "EPIC-001" in meta["epic_assets"]
    assert "asset1.txt" in meta["epic_assets"]["EPIC-001"]
