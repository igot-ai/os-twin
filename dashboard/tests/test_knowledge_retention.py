...
def test_refresh_namespace_returns_job_ids(
    ...
    """Test that refresh_namespace returns job IDs for completed imports."""
    ns_name = "refresh-ns"
    namespace_manager.create(ns_name)
    
    # Add completed import
    meta = namespace_manager.get(ns_name)
    assert meta is not None
    
    completed_import = ImportRecord(
        folder_path="/path/to/import",
        started_at=datetime.now(timezone.utc),
        finished_at=datetime.now(timezone.utc),
        status="completed",
    )
    namespace_manager.append_import(ns_name, completed_import)
    
    # Mock import_folder to return a job ID
    with patch.object(knowledge_service, 'import_folder', return_value='job-123'):
        job_ids = knowledge_service.refresh_namespace(ns_name)
...
def test_refresh_namespace_skips_failed_imports(
    ...
    """Test that refresh skips non-completed imports."""
    ns_name = "refresh-ns-2"
    namespace_manager.create(ns_name)
    
    # Add failed import
    failed_import = ImportRecord(
        folder_path="/failed/import",
        started_at=datetime.now(timezone.utc),
        finished_at=datetime.now(timezone.utc),
        status="failed",
    )
    namespace_manager.append_import(ns_name, failed_import)
    
    # Mock import_folder to track calls
    with patch.object(knowledge_service, 'import_folder') as mock_import:
        job_ids = knowledge_service.refresh_namespace(ns_name)
    
    # Should not have called import_folder for failed import
    assert len(job_ids) == 0
    mock_import.assert_not_called()


def test_refresh_namespace_empty_namespace(
    knowledge_service: KnowledgeService,
    namespace_manager: NamespaceManager,
):
    """Test that refresh on empty namespace returns empty list."""
    ns_name = "empty-ns"
    namespace_manager.create(ns_name)
...
def test_refresh_namespace_nonexistent_raises(
    knowledge_service: KnowledgeService,
):
    """Test that refresh on non-existent namespace raises error."""
    from dashboard.knowledge.namespace import NamespaceNotFoundError
    
    with pytest.raises(NamespaceNotFoundError):
        knowledge_service.refresh_namespace("nonexistent")


def test_refresh_namespace_uses_force(
    knowledge_service: KnowledgeService,
    namespace_manager: NamespaceManager,
):
    """Test that refresh passes force=True to import_folder."""
    ns_name = "force-refresh-ns"
    namespace_manager.create(ns_name)
    ...
    with patch.object(knowledge_service, 'import_folder') as mock_import:
        mock_import.return_value = 'job-123'
        knowledge_service.refresh_namespace(ns_name)
        
        # Should have called with force=True
        mock_import.assert_called_once()
        call_kwargs = mock_import.call_args
        assert call_kwargs[0][2].get('force') is True  # Third arg is options
