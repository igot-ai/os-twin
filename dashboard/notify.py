...
def get_config():
    ...
    try:
        ...
        if (
            config.get("chat_id")
            and config.get("chat_id") != "test_chat"
            and str(config["chat_id"]) not in config["authorized_chats"]
        ):
...
def save_config(bot_token: str, chat_id: str):
    ...
    if (
        chat_id
        and chat_id != "test_chat"
        and str(chat_id) not in config["authorized_chats"]
    ):
